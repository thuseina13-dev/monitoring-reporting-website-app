...
const StepperForm = ({ id }: { id: string }) => {
  ...
  const onSubmit = async (data: TaskTemplateForm) => {
    try {
      ...
      console.log('Updating Payload:', JSON.stringify(finalData, null, 2));
      await taskDefinitionService.updateTaskDefinition(id, finalData);
      queryClient.invalidateQueries({ queryKey: ['task-definitions'] });
      toast.show('Berhasil', { message: 'Template Berhasil Diperbarui!', type: 'success' });
      router.push('/manager/task-templates');
    } catch (error: any) {
      console.error(error);
      const serverError = error?.response?.data;
      let errMsg = 'Gagal memperbarui template';
      if (serverError) {
  ...
  return (
    <YStack flex={1} backgroundColor="$background">
      <ListHeader title="Edit Template Tugas" />
      <YStack padding="$4" paddingBottom="$0">
        <Text color="$colorFocus" textAlign="center" fontSize={14}>
          Lengkapi data di bawah ini untuk memperbarui template tugas.
        </Text>
      </YStack>
...
export default function EditTaskTemplateScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const { data: template, isLoading, isError } = useQuery({
    queryKey: ['task-definition', id],
    queryFn: () => taskDefinitionService.getTaskDefinitionById(id),
    enabled: !!id,
  });
  ...
  if (isLoading) {
    return (
      <YStack flex={1} justifyContent="center" alignItems="center" backgroundColor="$background">
        ...
        <Text mt="$2" color="$colorFocus">Memuat detail template...</Text>
      </YStack>
    );
  }

  if (isError || !template || !template.data) {
    return (
      <YStack flex={1} justifyContent="center" alignItems="center" backgroundColor="$background" padding="$4">
        <Text color="$red10" textAlign="center">Gagal memuat template tugas. Silakan coba lagi.</Text>
      </YStack>
    );
  }

  return (
    <FormBuilderProvider defaultValues={template.data}>
      <StepperForm id={id} />
    </FormBuilderProvider>
  );
}
