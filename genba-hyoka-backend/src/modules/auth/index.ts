...
export const changePasswordHandler = async ({ body, currentUser }: { body: any, currentUser: any }) => {
  ...
  if (userId && userId !== currentUser.id) {
    ...
    // Blokade: Jangan izinkan admin/siapapun mereset password Super Admin (Master Sistem) via fitur ini
...
// ── Protected Routes ───────────────────────────────────────
authModule.group('', (app) => 
  app
    .use(jwtGuard)
    .patch('/change-password', changePasswordHandler, changePasswordDocs)
);